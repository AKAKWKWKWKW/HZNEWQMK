...
🏵️ Привет! Я бот GEEF-GARANT, я помогу тебе безопасно и качественно провести твою сделку. 

🧅 Ради вашей безопасности, в боте размещен SCAM LIST, который поможет не наткнуться на мошенников. Наша цель - сделать все круто!

🍁 Я буду очень рад выслушать ваши предложения и жалобы. Сделаем все, что бы вы безопасно проводили сделки.''',parse_mode='HTML',disable_web_page_preview = True, reply_markup=keyboards.main)
			if message.text[7:] != '':
...
def garant(message):
	try:
		...
		if int(balanss[0]) >= int(message.text[8:].split(' ')[1]):
			...
			bot.send_message(message.chat.id, f'''🧩  {user} у тебя новая сделка от @{message.from_user.username} на сумму {money} RUB, Перейди в свои сделки!''',parse_mode='HTML')
			bot.send_message(message.chat.id, f'''🍏 Сделка #G{id_sdelka} от @{message.from_user.username} для @{user}

💰 Сумма сделки: {money} RUB''')
		else:
			bot.reply_to(message, '🥑 Недостаточно средств, перейдите свой профиль и пополните баланс!')

	except:	
		bot.reply_to(message, '🌿 Пользователь не найден в базе, пусть напишет любое сообщение.')
...
def send_text(message):
	...
	if message.chat.type == 'private':
		...
		if str(name[0]) == str(username):
			...
			bot.send_message(message.chat.id, '♻️ Ваш юзернейм был обнавлен!')
		...
		if str(status[0]) == str('Активен'):
			...
			if message.text.lower() == '/admin':
				if message.chat.id == config.admin:
					msg = bot.send_message(message.chat.id, '<b>🫅 Привествую, повелитель</b>',parse_mode='HTML', reply_markup=keyboards.admin)
					return
...
⚠️В чате должно быть от 300 участников.''', reply_markup=keyboard)
			...
			elif message.text.lower() == '♻️ мои сделки':
				keyboard = types.InlineKeyboardMarkup()
				...
				msg = bot.send_message(message.chat.id, f'♻️ Введите никнейм в формате username как в профиле!',parse_mode='HTML', reply_markup=keyboards.otmena)
